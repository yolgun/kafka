...
class LogTest extends JUnitSuite {
  ...
  def testAppendAndReadWithSequentialOffsets() {
    ...
    assertEquals("Reading beyond the last message returns nothing.", 0, log.read(values.length, 100, None).logBuffer.shallowIterator.asScala.size)
  }
  ...
  def testCompressedMessages() {
    ...
    def read(offset: Int) = log.read(offset, 4096).logBuffer.records
  ...
  def testMessageSizeCheck() {
    val first = TestUtils.logBufferWithValues(Record.CURRENT_MAGIC_VALUE, CompressionType.NONE,
      "1".getBytes, "2".getBytes)
    val second = TestUtils.logBufferWithValues(Record.CURRENT_MAGIC_VALUE, CompressionType.NONE,
  ...
  def testAppendMessageWithNullPayload() {
    ...
    val head = log.read(0, 4096, None).logBuffer.records.next()
