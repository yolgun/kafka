...
class StreamsScalePartitionsTest(Test):
    """
    Simple benchmark of Kafka Streams.
    """

    def __init__(self, test_context):
        super(StreamsScalePartitionsTest, self).__init__(test_context)
    ...
    def test_scale_partitions(self, scale, num_partitions):
        ...
        self.load_driver = StreamsSimpleBenchmarkService(self.test_context, self.kafka,
                                                         self.num_records * scale, "true", "processstreamwithsink")
        ...
        for num in range(0, scale):
            self.driver[num] = StreamsSimpleBenchmarkService(self.test_context, self.kafka,
                                                             self.num_records/(scale), "false", "processstreamwithsink")
